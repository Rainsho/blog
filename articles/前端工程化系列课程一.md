...
![ngnr-structure](../assets/ngnr-structure.jpg)
...
这里所说的资源，是除了 JavaScript 以外的所有文件，包括 CSS 样式文件、图片文件、模版文件等等。前面说到了，webpack 在执行时会从入口模块开始构建依赖树，而构建依赖树需要将模块解析成 AST，对资源文件来讲是没办法通过 JavaScript 解析器进行解析的，所以需要借助 loader 就行转换。

loader 会将特定的模块交由特定的解析器进行解析，比如 `postcss-loader` 将模块交给 postcss 解析器进行解析。解析后的模块，如果能进一步转换成 JavaScript 可执行的模块，则可以选择继续转换，比如 `style-loader` 将 CSS 样式转成 `style` 标签由 JavaScript 插入 DOM。还有一种情况，比如较大的图片文件，没有必要进行转换，这个时候 loader 通常就直接按配置将文件输出到指定目录，然后把模块内对文件的引用(`import`)转换成表明路径的字符串或者直接返回空文本文件。
